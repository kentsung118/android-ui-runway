package com.kent.android.slim.sample.letv.desktopmanager.anim;
...
/**
 * this is a copy of {@link DefaultItemAnimator}
 * adding {@link ItemAnimatorFinishedListener}
 * to watch the animation finish action
...
public class RecyclerAnimator extends SimpleItemAnimator {
    ...
    private ArrayList<ItemAnimatorFinishedListener> mFinishedListenersSelf = new ArrayList<ItemAnimatorFinishedListener>();
    private ArrayList<ItemAnimatorStartedListener> mStartedListenersSelf = new ArrayList<ItemAnimatorStartedListener>();
    ...
    private static class ChangeInfo {
        ...
        public String toString() {
            return "ChangeInfo{" + "oldHolder=" + oldHolder + ", newHolder=" + newHolder
    ...
    public boolean animateAdd(final ViewHolder holder) {
        ...
        // let the focus animation go first
        // doing this in {@link onAddStarting()} is not early enough;
        dispatchAnimationsStartSelf(holder);
    ...
    private void dispatchFinishedWhenDone() {
        if (!isRunning()) {
            ...
            dispatchAnimationsFinishedSelf();
        }
    }

    /**
     * when animation finished，inform all listeners
     */
    private void dispatchAnimationsFinishedSelf() {
        final int count = mFinishedListenersSelf.size();
        for (int i = 0; i < count; ++i) {
            mFinishedListenersSelf.get(i).onAnimationsFinished();
        }
        mFinishedListenersSelf.clear();
    }

    private void dispatchAnimationsStartSelf(ViewHolder holder) {
        final int count = mStartedListenersSelf.size();
        for (int i = 0; i < count; ++i) {
            mStartedListenersSelf.get(i).onAnimationsStarted(holder);
        }
        mStartedListenersSelf.clear();
    }
    ...
    public void addAnimationsStartedListener(ItemAnimatorStartedListener startedListener) {
        mStartedListenersSelf.add(startedListener);
    }
    ...
    public interface ItemAnimatorStartedListener {
        void onAnimationsStarted(ViewHolder holder);
    }
}
