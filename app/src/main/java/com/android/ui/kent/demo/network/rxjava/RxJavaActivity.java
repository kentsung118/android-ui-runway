...
public class RxJavaActivity extends BaseActivity {
    ...
    void singleRequestQueue() {
        ...
        api.getUser("john").subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())

                .doOnNext(new Consumer<User>() {
                    @Override public void accept(User user) throws Exception {
                        //先根據结果去做一些操作
                        Toast.makeText(mContext, "get user.name:" + user.name, Toast.LENGTH_SHORT)
                                .show();
                    }
                }).doOnError(new Consumer<Throwable>() {
            @Override public void accept(Throwable throwable) throws Exception {
                Log.d(TAG, "query John Fail!! throwable = " + throwable.getMessage());
                Toast.makeText(mContext, "john 查詢失敗 ( throwable = " + throwable.getMessage() + " )",
                        Toast.LENGTH_SHORT).show();
            }
        })

                .observeOn(Schedulers.io()).concatMap(new Function<User, ObservableSource<User>>() {
            @Override public ObservableSource<User> apply(User user) throws Exception {
            ...
            @Override public void accept(Throwable throwable) throws Exception {
                ...
                Toast.makeText(mContext, "jack 查詢失敗 ( throwable = " + throwable.getMessage() + " )",
                        Toast.LENGTH_SHORT).show();
            }
        }).observeOn(Schedulers.io()).concatMap(new Function<User, ObservableSource<User>>() {
            @Override public ObservableSource<User> apply(User user) throws Exception {
            ...
            @Override public void accept(Throwable throwable) throws Exception {
                ...
                Toast.makeText(mContext,
                        "wilson 查詢失敗 ( throwable = " + throwable.getMessage() + " )",
                        Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    void multipleRequestByZip() {
        ...
        Observable.zip(observable1, observable2, new BiFunction<User, User, String>() {
            @Override public String apply(User user, User user2) throws Exception {
                String result =
                        " query result user1.name =  " + user.name + ", user2.name = " + user2.name;
                return result;
            }
    ...
    private void testCustomObserverable() {
        ...
        t1.observeOn(AndroidSchedulers.mainThread())
                ...
                .concatMap(new Function<String, ObservableSource<Integer>>() {
                    @Override public ObservableSource<Integer> apply(String s) throws Exception {
                        return new T2();
                    }
                })
                .subscribe(new Observer<Integer>() {
